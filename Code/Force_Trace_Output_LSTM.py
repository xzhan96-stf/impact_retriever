...
from tensorflow.keras import optimizers, initializers, regularizers, callbacks
...
# Dir_results = '/home/jupyter/force/Results'
# Dir_data_X = '/home/jupyter/force/X/Signals'
# Dir_data_Y = '/home/jupyter/force/Y'
# Dir_code = '/home/jupyter/force/Code'
...
def buildBaseModel(hidden_unit, dropout, output_nodes, lr, initialization, regularization, loss='mean_squared_error'):
    model = Sequential([Input(shape = (145, 48)),
                        LSTM(hidden_unit, return_sequences = True, kernel_regularizer = regularizers.l2(0.01)),
...
def main(outcome, hidden_unit, epoch):
    ...
    Y1 = io.loadmat('Force_output_signal.mat')['signal_matrix'][:,[outcome_dict[outcome]],:]
    Y2 = io.loadmat('Force_mirror_output_signal.mat')['signal_matrix'][:,[outcome_dict[outcome]],:]
    Y = np.concatenate([Y1, Y2], axis=2).transpose((2, 1, 0))

    T1 = io.loadmat('Force_T.mat')['T'].reshape(-1,1)
    T2 = io.loadmat('Force_mirror_T.mat')['T'].reshape(-1,1)
    T = np.row_stack([T1, T2]).reshape(-1,)
    assert Y.shape[2] == 145
    assert Y.shape[1] == 1
    ...
    for repeat in range(0,20):
        ...
        X_train, Y_train, T_train = X[X_train_id,:], Y[Y_train_id,:], T[Y_train_id]
        X_train_val, Y_train_val = X[X_train_val_id,:], Y[Y_train_val_id,:]
        X_test, Y_test, T_test = X[X_test_id,:], Y[Y_test_id,:], T[Y_test_id]
        X_val, Y_val, T_val = X[X_val_id,:], Y[Y_val_id,:], T[Y_val_id]
        ...
        model, train_time, plt_val = modelFit(X_train_std, Y_train_std, X_val_std, Y_val_std, model, epoch = epoch, lr=lr, batch_size=512, augment = augment, verbose = True)
        ...
        MAL, RMSL, R2_train = evaluate_truncated(Y_train.squeeze(), Y_predict_train, T_train)
        MAV, RMSV, R2_val = evaluate_truncated(Y_val.squeeze(), Y_predict_val, T_val)
        ...
        MAE, RMSE, R2_test = evaluate_truncated(Y_test.squeeze(), Y_predict_test, T_test)
        print('Y_test shape: ', Y_test.squeeze().shape)
        ...
        specifics = 'New2024_Repeat' + str(repeat) + '_two-layer-BiLSTM_' + str(hidden_unit) + '_' + Ymethod + '_Dense'+str(Dense) + '_epoch' + str(epoch)
    ...
    group_name = 'New2024_YPreprocess'+ '_two-layer-BiLSTM_' + str(hidden_unit) + '_' + Ymethod + '_Dense'+str(Dense) + '_epoch' + str(epoch)
